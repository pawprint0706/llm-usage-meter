...
- **OpenRouter** — 크레딧 잔액과 총 충전 대비 사용률
...
트레이 아이콘은 **현재 선택한 탭의 서비스** 게이지를 바늘로 표시합니다(팝업을 닫은 뒤에도 마지막 선택 탭 유지). 서비스마다 가장 짧은 주기의 사용률을 기준으로 하고, 바늘은 카드와 마찬가지로 **남은 양**을 가리킵니다 — 가득 차서 시작하고, 사용할수록 바늘이 내려갑니다(코덱스는 5시간, OpenCode는 5시간 창 기준). 팝업을 한 번도 열지 않으면 탭 순서의 첫 서비스를 따릅니다. macOS에서는 template image로 처리되어 메뉴 바 색상에 자동으로 맞춰지고, Windows에서는 작업 표시줄 테마(`SystemUsesLightTheme`)에 따라 검정/흰색으로 바뀝니다.
...
- **크레딧**: 구매한 총 크레딧과 총 사용액으로 **남은 크레딧**을 표시합니다. 트레이 게이지는 **총 충전액 대비 사용률**을 가리키며, 100%는 잔액 0입니다. 크레딧을 충전하면 바늘이 내려갑니다. 남은 크레딧이 0 이하면 흐리게 표시합니다.
