...
- **Ollama** — 기본(월간 크레딧) 사용량과 모델별 요청 수, 추가 사용량 잔액
- **OpenRouter** — 현재 크레딧 잔액
...
- **탭**: 켜 둔 서비스마다 아이콘 탭 하나. 탭을 바꿔 해당 서비스의 사용량·크레딧을 보고, 마우스를 올리면 서비스 이름이 표시됩니다
- **카드**: 서비스 이름, 플랜 배지, `⋯` 서비스별 메뉴(사용량·통계 페이지 이동은 여기로만)

트레이 아이콘은 **현재 선택한 탭의 서비스** 게이지를 바늘로 표시합니다(팝업을 닫은 뒤에도 마지막 선택 탭 유지). 서비스마다 가장 짧은 주기의 사용률을 기준으로 하고, 바늘은 카드와 마찬가지로 **남은 양**을 가리킵니다 — 가득 차서 시작하고, 사용할수록 바늘이 내려갑니다(코덱스는 5시간, OpenCode는 5시간 창 기준). 다만 OpenRouter 탭은 사용 한도를 뜻하는 게이지가 없으므로 바늘 대신 공식 OpenRouter 로고를 표시합니다. 팝업을 한 번도 열지 않으면 탭 순서의 첫 서비스를 따릅니다. macOS에서는 template image로 처리되어 메뉴 바 색상에 자동으로 맞춰지고, Windows에서는 작업 표시줄 테마(`SystemUsesLightTheme`)에 따라 검정/흰색으로 바뀝니다.

카드의 사용량 바는 **연료 게이지**처럼 그려집니다: 제공량 전체가 초록으로 채워져 있고, 사용할수록 남은 만큼 줄어듭니다. 남은 양이 적어지면 주황(70% 이상 사용)→빨강(90% 이상 사용)으로 경고 색이 바뀝니다.
...
- **플랜 사용량**: API의 `used_percent` 기준으로 윈도우(5시간 / 주간 등)마다 **남은 사용률**과 초기화 카운트다운을 표시합니다.
...
- **Go 플랜 사용량**: 콘솔이 알려주는 사용률(%)에 기간별 한도를 곱해 금액으로 환산하고, **남은 금액**으로 표시합니다. 기본 한도는 5시간 $12, 주간 $30, 월간 $60이며 설정 파일에서 바꿀 수 있습니다.
- **Zen 크레딧**: 크레딧 잔액, 이번 달 남은 금액과 한도, 자동 충전 설정을 표시합니다. 워크스페이스가 Zen 크레딧으로 결제되는 경우 Go 섹션에 안내 문구가 추가됩니다.
...
- **플랜 사용량**: 청구 주기 기준 전체·자동·API의 **남은 사용률**과 주기 종료 카운트다운을 표시합니다. 무제한 플랜이면 전체를 **무제한**으로 표시합니다.
- **지출**: 포함(included) 크레딧의 남은 금액/한도, 보너스 크레딧, 온디맨드(사용량 기반)의 남은 금액/한도를 표시합니다. 온디맨드가 꺼져 있으면 **꺼짐**으로 표시합니다.
...
- **기본 사용량**: 월간 크레딧의 **남은 금액**과 초기화 카운트다운을 표시합니다.
- **추가 사용량**: 플랜 포함 사용량을 소진한 뒤 쓰는 잔액을 표시합니다. 잔액이 0이면 흐리게 표시합니다.
- 아래 **모델 목록**: 이번 달 모델별 요청 수를 제목 없이 잔액 아래 나열합니다.
...
- **크레딧**: 구매 총액 대비 사용률이나 막대 그래프 없이 **현재 잔액**을 달러로 크게 표시합니다. 그 아래에는 더 작은 보조 항목으로 **누적 충전액**을 표시합니다. 잔액이 0 이하면 흐리게 표시합니다. OpenRouter 탭을 선택한 경우 트레이에는 게이지 대신 공식 OpenRouter 로고가 표시됩니다.

크레딧·활동·로그 페이지는 `⋯` 메뉴의 해당 항목으로만 이동합니다. 키 등록 전에는 붙여넣기/입력도 같은 메뉴에 있고, 등록 후에는 로그아웃만 남습니다.

### OpenRouter API 키

크레딧 조회는 공식 API(`GET /api/v1/credits`)를 사용합니다. 공식 문서에는 "Management 키 필요"로 표기돼 있지만, 실제로는 일반 채팅용 API 키(`sk-or-v1-...`)로도 자기 계정 크레딧 조회가 됩니다(2026-09 기준 확인).

1. <https://openrouter.ai/settings/keys> 에서 API 키를 생성합니다.
2. 키 값(`sk-or-...`)을 복사합니다.
3. 카드의 **API 키 입력...** 또는 **클립보드에서 API 키 붙여넣기**로 등록합니다. (`Bearer `, `Authorization: `, 따옴표는 자동으로 정리됩니다.)

조회 권한이 막힌 키면 403 안내가 표시되며, 이 경우 키는 지워지지 않습니다 — `settings/management-keys`에서 Management 키(`sk-or-mgmt-...`)를 발급해 대신 등록하면 됩니다. 키가 거부되면(401) 저장된 키를 지우고 알림을 띄웁니다.

## 자격 증명 보안

Codex OAuth 토큰, OpenCode 세션 키, Cursor에서 붙여넣은 세션 토큰, Ollama 세션 쿠키, OpenRouter API 키는 OS 보안 저장소에 보관합니다.
...
{
  "refresh_interval": 10,
  "provider_order": ["codex", "opencode", "cursor", "ollama", "openrouter"],
  "providers": {
    "codex": { "enabled": true },
    "opencode": {
      "enabled": true,
...
테스트는 Codex 사용량·크레딧·재설정권 파싱과 401/403 단일 재시도, OpenCode 콘솔 SSR 파서와 Zen 빌링 추출, Cursor 사용량·지출 파싱과 세션 토큰 정리, Ollama 설정 페이지 파싱과 세션 쿠키 정리, OpenRouter 크레딧 응답 파싱과 API 키 정리, 자격 증명 압축·분할 저장, 설정 파일 읽기/쓰기, 금액·기간 표시, 팝업 렌더링(오프스크린 Qt)을 검사합니다. 실제 계정이나 네트워크 호출은 사용하지 않습니다.
...
- **OpenRouter에서 403 안내가 표시됨**: 등록한 키로는 크레딧 조회가 막힌 것입니다. `management-keys` 페이지에서 Management 키(`sk-or-mgmt-`)를 발급해 다시 등록하세요.
- **OpenRouter 키가 거부됨(401)**: 키가 삭제되고 카드가 로그아웃 상태가 됩니다. `settings/keys`에서 새 키를 발급해 등록하세요.
...
이 앱은 대부분 문서화되지 않은 내부 엔드포인트를 사용합니다. OpenRouter만 공식 문서화된 엔드포인트입니다.
...
GET https://openrouter.ai/api/v1/credits               (공식 문서화 · 일반 API 키도 허용)
```
...
LLM Usage Meter는 독립적으로 제작된 유틸리티입니다. OpenAI, OpenCode, Cursor, Ollama, OpenRouter의 공식 앱이 아니며, 해당 회사가 보증·후원하거나 제휴한 프로젝트가 아닙니다.

ChatGPT, Codex, OpenAI, Blossom 로고와 OpenCode·Cursor·Ollama·OpenRouter 관련 표장은 각 소유자의 상표 또는 자산입니다. `assets/codex-blossom.ico`, `assets/cursor-cube.svg`, `assets/ollama-icon.svg`, `assets/openrouter-icon.svg`는 모니터링 대상 서비스를 카드에서 식별하기 위한 목적으로만 사용하며, 단색 glyph로 비율을 유지해 크기만 조정합니다. 이 앱 자체의 브랜드로 표시하지 않습니다.
